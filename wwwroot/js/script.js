...
const updateLivesDisplay = () => {
    livesDisplay.innerText = `You Have ${incorrectGuesses} points`;
};
...
const handleGuess = (letter, button) => {
    ...
    if (selectedWord1.includes(letter) || selectedWord2.includes(letter) || selectedWord3.includes(letter) || selectedWord4.includes(letter) || selectedWord5.includes(letter)) {
        ...
        updateLivesDisplay
        if (maxLives = 0) {
            message.innerText = `Game over! The words were ${selectedWord1 && selectedWord2 && selectedWord3 && selectedWord4 && selectedWord5}`;
        }
    }
};
